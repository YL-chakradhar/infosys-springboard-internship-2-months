...
class CustomerViewSet(viewsets.ModelViewSet):
    ...
    def register(self, request):
        ...
        serializer = self.get_serializer(data=request.data)# initializing the serializer 
...
class CakeViewSet(viewsets.ModelViewSet):
    queryset = Cake.objects.all()#retrieving all the instances from the cake 
...
class CakeCustomizationViewSet(viewsets.ModelViewSet):
    ...
    def get_custom(self, request, pk):
        ...
        return Response({'message':'no customization found for the user'},status=status.HTTP_204_NO_CONTENT)#the process has done but no content"204"
...
def schedule_email(send_time,subject, message, recipient_list):
        ...
        scheduler.enter(delay, 1, send_mail, argument=(subject, message, 'yerralakshmichakradhar5@gmail.com', recipient_list))
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def partial_update(self, request, pk=None):
        ...
        if payment_method:
            ...
            print(recipient)
            try:
                send_mail(subject, message, 'yerralakshmichakradhar5@gmail.com', [recipient])
            except Exception as e:
                return Response({'message': 'Failed to send email', 'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            order.save()
    ...
    def  delivery_tracking(self,request,id):
        ...
        sweetspot_location= "Duvvada,Andhra pradesh,India"
...
class StoreViewSet(viewsets.ModelViewSet):
    ...
    def get_email_from_request(self, request):
        return request.data.get('adminemail') 
